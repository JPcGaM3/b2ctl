...
จำลอง server 1 เครื่อง 6 disks เป็น `state.json` แล้วรัน **b2ctl ตัวจริง** กับมันได้บน laptop
โดยไม่ต้องมี hardware / SSH / root. b2ctl คุย hardware ผ่าน subprocess ทั้งหมด — harness นี้
แค่วาง **fake binaries** (`bin/zpool`, `lsblk`, `sas2ircu`, `storcli`, `smartctl`, ...) บน PATH
...
python3 sim/simctl mode raid       # สลับเป็น RAID-mode (bay via storcli/perccli)
...
- status / --json / pool-aware summary, check / update / config
- watch ทุก hotkey: r a o s d t n l q
- swap / replace / offload / demote / create pool — **state เปลี่ยนจริง**
- hot-plug (pull/insert), spare auto-replace, resilver progress
...
| `_simstate.py` | state load/save + helpers + default 6-disk layout |
