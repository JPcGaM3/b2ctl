...
**ผลรอบนี้ (dev machine):** `508 passed, 0 failed` ✅ *(v0.11.0-itmode: `[a]ssign` multi-select/batch — +13 tests: watch batch-parse/dedupe/reject/mixed-refusal/perc+zfs routing, `_pick_indices` F-052 guard+dedupe, raid_actions assign_perc_batch JBOD-all/create-one-VD/hotspare-all/IT-gate/multi-controller-refusal. Review pass also fixed a pre-existing F-052 gap in `[n]ew-pool`/`[e]xtend` where `0` selected the LAST disk)* · *(v0.10.0-itmode: multi-disk background burn-in — +34 tests: parse_selftest/ETA, start_scan Popen, scan_progress, state save/load, _pid_alive, burnin_snapshot, live_view, run_multi re-entrancy, smart self-test fields, ui fmt_eta/render_burnin, watch multi-select, cli nargs/--status)*

| ID | Scenario | Expected | Status | Actual | Comment |
|----|----------|----------|:------:|--------|---------|
| E1 | Test suite รันได้ | suite รันจบ ไม่ error การ import | ✅ | tests collected, รันจบ | OK |
| E2 | Pass rate | tests ผ่านทั้งหมด | ✅ | **508 passed / 0 failed** (+4 subtests) | …prior deltas… ; +87 (v0.9.0) Fable5 audit ; +34 (v0.10.0) burn-in: parse_selftest+ETA, start_scan (Popen, no -w, start_new_session), scan_progress+_parse_badblocks_log, _pid_alive reap, state save/load under safety.LOG_DIR, burnin_snapshot, live_view finish/Ctrl-C, run_multi start/re-entrant/dry-run, _finish prune+scan-bad→WARN, status_view, smart selftest fields, ui fmt_eta/TEST%-cell/render_burnin_view, watch multi-select+re-attach, cli nargs+--status ; +13 (v0.11.0) assign multi-select/batch: watch parse+dedupe+reject+mixed-refusal+perc/zfs routing, `_pick_indices` F-052 guard+dedupe, raid_actions assign_perc_batch (JBOD-all/create-one-VD/hotspare-all/IT-gate/multi-controller-refusal) |
