...
**ผลรอบนี้ (dev machine):** `249 passed, 0 failed` ✅ *(เพิ่ม L2ARC/SLOG aux vdevs, RAID10 create, disk burn-in, NVMe bay-map by by-id/serial, watch RAID-volumes parity, sim NVMe — บน 215 เดิม)*

| ID | Scenario | Expected | Status | Actual | Comment |
|----|----------|----------|:------:|--------|---------|
| E1 | Test suite รันได้ | suite รันจบ ไม่ error การ import | ✅ | tests collected, รันจบ | OK |
| E2 | Pass rate | tests ผ่านทั้งหมด | ✅ | **249 passed / 0 failed** | +26 (v0.8.0) add_cache/add_log/remove_vdev, raid10, burnin; +6 (v0.8.1) nvme by-id/serial remap; +2 (v0.8.2) watch RAID-volumes render |
