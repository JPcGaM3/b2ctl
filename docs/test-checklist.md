...
**ผลรอบนี้ (dev machine):** `279 passed, 0 failed` ✅ *(เพิ่ม unified Storage summary, +v0.8.5 bay_map/ssd_spec directory-independence + `b2ctl update` resource sync)*

| ID | Scenario | Expected | Status | Actual | Comment |
|----|----------|----------|:------:|--------|---------|
| E1 | Test suite รันได้ | suite รันจบ ไม่ error การ import | ✅ | tests collected, รันจบ | OK |
| E2 | Pass rate | tests ผ่านทั้งหมด | ✅ | **279 passed / 0 failed** | +26 (v0.8.0) add_cache/add_log/remove_vdev, raid10, burnin; +6 (v0.8.1) nvme by-id/serial remap; +2 (v0.8.2) watch RAID-volumes render; +5 (v0.8.3) install parity; +v0.8.4 storage summary; +13 (v0.8.5) bay_map/ssd_spec path precedence + `b2ctl update` sync (created/current/customized-kept/force) |
