...
ครั้งล่าสุดไม่ว่าใครสั่ง — burn-in (`[b]`), health-check ใน `[m]aint`, หรือ `smartctl -t
long` ที่รันเอง วิธีสั่งทดสอบดู `[m]aint` ด้านล่าง
...
`2d ago` ถ้าว่าง/`-` = ยังไม่มีบันทึก — สั่งได้ด้วย `[m]aint` หรือ `b2ctl scrub|trim <pool>`
...
**`size to use per disk` (over-provision, v0.17.0):** กด Enter = ใช้ **ดิสก์ทั้งลูก** (ค่า
...
**`autoscrub` (v0.17.0, ค่าเริ่มต้น = OFF):** scrub อ่านทุกบล็อกที่ใช้งาน ตรวจ checksum และ
ซ่อมข้อมูลเสียหายเองบน pool ที่มี redundancy **เลือก `[1] on`** เพื่อเปิด timer scrub ราย
เดือน (`zfs-scrub-monthly@<pool>.timer`) ค่าเริ่มต้นคือ **`[2] off`** — b2ctl ถือว่า **การ
scrub ด้วยมือคือแนวทางหลัก** (`[m]aint` / `b2ctl scrub`) ดังนั้น pool ใหม่จะ *ไม่มี* scrub
ตามกำหนดจนกว่าจะเลือกเปิดเอง เมื่อ off b2ctl จะเตือนและคอลัมน์ `SCRUB` จะโชว์ว่า scrub
ล่าสุดเก่าแค่ไหน — ควรสั่งเป็นระยะ (`autotrim` เป็นตัวเลือกแยกและค่าเริ่มต้นก็ off เช่นกัน)
...
- **`size to use per device` (over-provision, v0.17.0):** กด Enter = ใช้ทั้งลูก หรือใส่ขนาด
...
**ใช้เมื่อ:** สั่ง scrub หรือ TRIM บน pool ด้วยมือ (แนวทางหลักใน v0.17.0 เพราะ scrub ตาม
กำหนดกลายเป็น opt-in) หรือสั่ง long self-test เพื่ออัปเดตคอลัมน์ HEALTH_CHK ของดิสก์
...
- **trim** — บอก SSD ว่าบล็อกไหนว่าง (ข้ามได้ถ้า pool ตั้ง `autotrim=on` ซึ่ง trim ต่อเนื่องอยู่
  แล้ว) ความคืบหน้าหยาบ (ZFS เปิดข้อมูล trim จำกัด)
- **health-check** — สั่ง `smartctl -t long` บนดิสก์ที่เลือก (ดิสก์ที่อ่านได้ทุกลูก) รันบนตัวดิสก์
  **หลักนาทีถึงหลักชั่วโมง** และ **ไม่บล็อก** b2ctl ผลลงใน self-test log ของดิสก์และโผล่ที่
  คอลัมน์ **HEALTH_CHK** ในการ refresh ครั้งถัดไป เป็น long test ตัวเดียวกับที่ burn-in ใช้
- **Ctrl-C = detach ไม่ใช่ยกเลิก** ระหว่างดู scrub/trip สด Ctrl-C พากลับ prompt แต่ **kernel
...
- ทุก scrub/trim/health-check ถูกบันทึกลง **ประวัติ maintenance** (`maint.jsonl`) ดูด้วย
  `b2ctl maint --log`
- CLI: `b2ctl scrub <pool>` · `b2ctl trim <pool>` (ไม่ใส่ pool = ถาม) · `b2ctl maint --log
  [--last N]`

---

### 6.9 `b` — Burn-in (ตรวจดิสก์ก่อนเข้าใช้ · หลายลูก + เบื้องหลัง)
...
| `sudo b2ctl scrub [<pool>]` | สั่ง scrub ด้วยมือ (ตรวจ checksum + self-heal); ไม่ใส่ pool = ถาม |
| `sudo b2ctl trim [<pool>]` | สั่ง TRIM ด้วยมือ (คืนบล็อกว่างให้ SSD) |
...
| `n` | new-pool | สร้าง pool ใหม่ (ถาม over-provision size + autoscrub) |
| `e` | extend | เพิ่ม/ถอด/**ซ่อม** L2ARC cache หรือ SLOG log (ถาม SLOG topology + size) |
| `m` | maint | maintenance ด้วยมือ — scrub / trim (ต่อ pool) / health-check (long self-test) |
...
**autotrim** และ **autoscrub** เป็นสองตัวเลือกแยกกัน (v0.17.0). b2ctl ตั้ง maintenance
ผ่าน **systemd timer ของ distro** (มากับ `zfsutils-linux`) ราย pool:
- **autoscrub** — *ค่าเริ่มต้น **off***. `[1] on` เปิด `zfs-scrub-monthly@<pool>.timer`.
  เมื่อ **off** pool จะ **ไม่มี scrub ตามกำหนด** — การ scrub ด้วยมือคือแนวทางหลัก (`[m]aint`
  / `b2ctl scrub`) b2ctl จะเตือนและคอลัมน์ SCRUB จะโชว์ว่า scrub ล่าสุดเก่าแค่ไหน *(นี่กลับ
  ทิศจากเดิมที่ scrub รันเสมอ — ดู ADR-003 pool ที่ไม่เคย scrub อาจสะสม bitrot เงียบ ๆ ควร
  scrub สม่ำเสมอหรือเปิด autoscrub)*
- **autotrim** — *ค่าเริ่มต้น off* — เปิด `zfs-trim-monthly@<pool>.timer`; เลือก `on` =
  trim ต่อเนื่องโดย ZFS เอง จึงไม่ตั้ง trim timer
...
- **Over-provision (v0.17.0):** ทั้ง cache/log ถาม `size to use per device` (Enter = ทั้งลูก,
  ใส่ขนาดเช่น `32G` = แบ่งพาร์ทิชันแล้วส่งพาร์ทิชันให้ ZFS เหลือ spare area ช่วยอายุ SSD).
  CLI: `--size 32G` บน `cache-add`/`log-add`
